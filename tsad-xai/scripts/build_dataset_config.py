...
# Rule sets are versioned so an amendment is never a silent edit. v1 is the
# original pre-registration; v2 is AMENDMENT-001 (see docs/AMENDMENT-001.md),
# adopted 2026-09-07 while results/ held synthetic smoke tests only.
RULESETS = {
    "v1": {"ar_lo": 0.004,  "dedup": False},
    "v2": {"ar_lo": 0.0005, "dedup": True},
}


def _dedup_recordings(keep, rng):
    """R8 -- collapse FOO / DISTORTEDFOO / NOISEFOO to one representative.

    They are one recording with one anomaly interval, so they are not
    independent samples. Which representative survives is drawn at random from
    the group rather than always preferring the undistorted one: DISTORTED and
    NOISE variants are the archive's own difficulty ladder, and always keeping
    the easiest would bias the sample toward trivial series.
    """
    if "file" not in keep:
        return keep
    grp = {}
    for i, fn in zip(keep.index, keep.file):
        m = UCR_BASE.match(fn)
        grp.setdefault((m.group(1), m.group(3), m.group(4)) if m else fn,
                       []).append(i)
    pick = [ix[0] if len(ix) == 1 else ix[rng.integers(len(ix))]
            for ix in grp.values()]
    return keep.loc[sorted(pick)]


def apply_rules(df, n=40, seed=0, strat="domain", ruleset="v2"):
    rs = RULESETS[ruleset]
    d = df.dropna(subset=["length"]).copy()
    d["R2"] = d.anomaly_ratio.between(rs["ar_lo"], 0.10)
    ...
    if rs["dedup"]:
        # R8: one series per (recording, anomaly interval). Applied BEFORE the
        # balanced draw, so dedup cannot bias which stratum loses members.
        keep = _dedup_recordings(keep, rng)
    picked = []
    if not keep.empty:
...
def main():
    ...
    ap.add_argument("--ruleset", choices=tuple(RULESETS), default="v2",
                    help="v1: original pre-registration. v2: AMENDMENT-001 "
                         "(R2 lower bound 0.0005, R8 dedup). Default v2.")
    ...
    print(f"scanned {len(df)} files  (format={a.format}, ruleset={a.ruleset},"
          f" stratify on {strat})")
    ...
    with open(a.out, "w") as f:
        ...
        f.write(f"# ruleset {a.ruleset}: R2 ar in"
                f" [{RULESETS[a.ruleset]['ar_lo']},0.10];"
                f" R8 dedup={RULESETS[a.ruleset]['dedup']}\n")
        f.write(f"# rules: R3 len in [6000,900000];"
